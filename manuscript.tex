...
% ---------------------------------------------------------------------------
% Working manuscript, formatted for Molecular Systems Biology.
% Standard article class so it builds anywhere (pdflatex manuscript.tex).
% ---------------------------------------------------------------------------
...
\title{\bfseries A computational model of the interactions between formate and
insulin metabolism}
...
% ---------------------------------------------------------------------------
\section*{Abstract}
\noindent Formate, the circulating carrier of one-carbon units, supplies two of
the carbons of every purine ring, and cellular purine nucleotides are sensed by
mTORC1 --- the master regulator of the anabolic translation that the pancreatic
$\beta$-cell uses to make insulin. These links suggest an untested chain, formate
$\rightarrow$ purine nucleotides $\rightarrow$ mTORC1 $\rightarrow$ insulin
synthesis, connecting one-carbon nutrition to insulin production and diabetes
risk. We formalise this chain as a computational kinetic model, calibrated to
independent islet data; the model itself --- a quantitative substrate for
reasoning about formate and insulin --- is the central contribution.

\noindent Systemically, raising formate increases the purine pool, mTOR activity
and insulin synthesis, while loss of mitochondrial formate production cuts insulin
synthesis by roughly half with a parallel fall in uric acid. Embedded in a
whole-body glucose--insulin loop, this central action raises plasma insulin,
distinguishing it from a peripheral, insulin-independent effect on glucose uptake.
Adding an adipose compartment shows that obesity and an insulin-secretion defect
drive blood formate in opposite directions --- reconciling human cohorts that
disagree in sign --- whereas food formate content does not predict diabetes risk,
because the $\beta$-cell is already formate-replete. A gut-microbiome compartment
that returns urate carbon as formate closes a delayed positive-feedback
``urate cycle''.

\noindent The retina is a distinct case: it produces its own insulin behind the
blood-retinal barrier, so retinal insulin tracks blood one-carbon rather than
plasma insulin. Self-regulation makes the eye a homeostat that holds retinal
insulin near an optimum; because excess retinal insulin is pathological, retinal
health is an inverted-U in formate, and methanol optic toxicity emerges as an
acute one-carbon overload --- without invoking inhibition of the respiratory
chain. The model turns these maps into testable interventions: serine/formate
supplementation benefits only the formate-deficient diabetic, and in the eye,
inhibiting local formate synthesis normalises insulin-excess retinopathy whereas
inhibiting insulin synthesis rescues methanol toxicity --- a paired,
mechanism-discriminating prediction.
...
synthesis of new hormone (Rorsman \& Ashcroft, 2018). Proinsulin biosynthesis is
...
control (Itoh \& Okamoto, 1980). Such rapid, large-scale anabolic translation is
the domain of mTOR complex~1, the master regulator that gates protein synthesis to
nutrient and energy supply (Saxton \& Sabatini, 2017); accordingly, mTORC1
...
resistance and so raise the secretory demand on the $\beta$-cell (Kahn, Hull \&
Utzschneider, 2006). Diabetes nevertheless emerges only when the $\beta$-cell can
...
and brain (DeFronzo, 2009; Roden \& Shulman, 2019). Because insulin
...
and gut-microbial sources (Ducker \& Rabinowitz, 2017; Pietzke, Meiser \& Vazquez,
2020). Two carbons of every purine ring derive from formate, so formate
availability constrains de novo purine synthesis (Oizel \textit{et al}, 2020). In
...
status rather than by smoking or alcohol (Brosnan \textit{et al}, 2018), and
circulating formate has been reported to stratify with cancer and diabetes
(Pietzke \textit{et al}, 2019).

These strands converge on a testable chain. Cellular
purine nucleotide levels are directly sensed by mTORC1 (Hoxhaj \textit{et al},
2017), the very complex that drives insulin translation. If formate feeds the
...
insulin peptide synthesis}. We proposed this chain informally (\textit{The Spice
of Life}, 2020); it has not been tested directly. Here we build a computational
...
(Detimary \textit{et al}, 1998), with the guanine-nucleotide pool rising in
parallel (Detimary 1996) and physiological ATP (0.6$\rightarrow$2.3~mM).
...
adenylosuccinate/S-AMP (\textbf{$\times$3.4}), reproducing Gooding \textit{et al}
(2015); this required an explicit, saturable S-AMP pool. (3)~\emph{mTOR
...
insulin content to \textbf{50\%} of wild type (Ni \textit{et al}, 2017).
...
index ${\sim}5$--$6$ (Nunemaker \textit{et al}, 2006) --- giving
...
(Pietzke \textit{et al}, 2019). Providing dietary formate rescued insulin
...
glucose uptake (Carp\'ene \textit{et al}, 2019). We embedded the $\beta$-cell as
...
($\beta$-cell synthesis) mechanism of \textit{The Spice of Life}, whereas
...
A large prospective cohort (Takase \textit{et al}, 2025; TMM CommCohort,
...
\emph{either} low formate (deficiency-route diabetes; Pietzke 2019) \emph{or}
high formate (secretion-defect diabetes; Takase 2025), and the two are told apart
...
\begin{figure}[t]
  ...
  association of Takase \textit{et al} (2025) as reduced insulin-driven
...
carries \emph{low} formate (Pietzke 2019) whereas incident diabetes in a
largely non-obese population carries \emph{high} formate (Takase 2025). We asked
...
\emph{source} whose activity and mass both rise in obesity (Tsushima \textit{et
al}, 2013). Coupling the $\beta$-cell, the adipocyte and shared blood pools
...
\begin{figure}[t]
  ...
  rises with obesity via adipose XOR (Tsushima 2013). Percentages are relative to
...
recognised as a major contributor to host purine homeostasis (Kasahara
\textit{et al}, 2023). The \emph{urate cycle} hypothesis (Vazquez, 2020) proposes
...
hypothesised route from urate to weight gain (Vazquez, 2020) --- which nominates
...
dominate (\textit{The Spice of Life}, p.~53): ${\sim}1000$~mg from 1~g creatine,
...
the formate-generation potential tabulated by Pietzke, Meiser \& Vazquez
(2020, Table~2) --- the formaldehyde released by demethylation of caffeine and
...
diabetes per serving --- coffee RR~0.91 (0.89--0.94; Ding \textit{et al}, 2014),
sugar-sweetened cola RR~1.13 and 100\% fruit juice RR~1.07 and artificially
sweetened (diet) cola RR~1.08 (Imamura \textit{et al}, 2015), red meat HR~1.10
per 100~g (Li \textit{et al}, 2024), and creatine glycaemically neutral in
randomized trials (fasting-glucose SMD~$+0.05$, HOMA-IR SMD~$-0.38$, both
non-significant; Delpino \& Figueiredo, 2022) --- the correlation between a food's
...
retinal model (\S\ref{ss:dietretina}).
...
\begin{figure}[t]
  ...
  Pietzke~2020 Table~2 (coloured by category, formate per serving annotated) plus
...
\begin{figure}[t]
  ...
  with a per-serving estimate (beverage formate contents from Pietzke~2020
...
\subsubsection{The eye: a self-regulating one-carbon compartment}\label{ss:eyecomp}
...
deficiency or resistance (Etchegaray \textit{et al}, 2023). The blood-retinal
...
SHMT2/MTHFD2 (Ben-Sahra \textit{et al}, 2016), and in islets the same ATF4
program that raises insulin content co-induces this serine-linked one-carbon
pathway (Pelligra \textit{et al}, 2023); phagocytosing RPE activates ATF4 as its
...
amplified by starvation (Etchegaray \textit{et al}, 2023). And insulin/IGF
signalling is capped by the mTORC1 $\rightarrow$ S6K1 $\rightarrow$ IRS-1
negative-feedback loop, shown directly in RPE cells (Leontieva \textit{et al},
2014); indeed the proliferative retinopathy of chronic ocular IGF-1 excess
(Ruberte 2004; Villacampa 2013) reads as loss of exactly this cap. Adding the
...
self-consistency/bifurcation analysis agree (Supplementary Fig.~S1): the
...
with the ocular tolerability of formate supplementation, Altaweel \textit{et al},
2009).
...
proliferative, neovascular retinopathy even without hyperglycaemia (Ruberte
\textit{et al}, 2004); sustained retinal IGF-I causes gliosis, oxidative stress
and neurodegeneration despite being neuroprotective at normal levels (Villacampa
\textit{et al}, 2013); and the ``early worsening'' of retinopathy on intensive
...
\subsubsection{Dietary formate and the retina}\label{ss:dietretina}
...
\subsubsection{Who benefits from formate/serine supplementation?}\label{ss:suppl}
...
lowers serine $\rightarrow$ formate flux (Tramonti \textit{et al}, 2021). We modelled
...
been developed as anticancer agents (e.g.\ DS18561882; Kawai \textit{et al}, 2019),
with MTHFD2 already flagged as an ocular drug target in retinoblastoma
(\textit{Exp Eye Res}, 2025). In the model an intravitreal MTHFD2 inhibitor scales down
...
feed-forward of $\S$\ref{ss:eyecomp}. We swept the inhibition from 0 to 100\%
(Fig.~\ref{fig:mthfd2}).
...
$\S$\ref{ss:eyecomp} (eye insulin ${\sim}2.46$, above the optimum) --- lowering local formate
...
thus the mirror image of formate/serine supplementation ($\S$\ref{ss:suppl}): it helps the
...
(Fraenkel \textit{et al}, 2008) and is available for the eye (intravitreal
...
\emph{Confrontation with existing data.} A survey of candidate test datasets
(supplementary \texttt{lit\-er\-a\-ture.md}) finds the mTOR $\rightarrow$ insulin arm
strongly supported (Raptor/\allowbreak TSC2 $\beta$-cell models; rapamycin) and folate/\allowbreak
one-carbon deficiency impairing $\beta$-cell insulin biosynthesis (Hsu 2013),
...
raises insulin \emph{content} but lowers secretion (Pelligra 2023) --- a
...
diabetes (opposite the ``low-formate'' reading of Pietzke 2019); as shown above
...
This section summarises the models; the Supplementary Material gives the complete
specification --- every state variable, rate law, ODE balance, parameter value with
provenance, calibration procedure and the monostability analysis (Fig.~S1).
...
are inherited from Formate-NUDT5 (Vazquez, submitted); GSIS parameters were
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}lr@{}}
    ...
    Quantity & Value \\
    \midrule
    ATP/ADP, 1$\rightarrow$10 mM glucose (Detimary 1998) & 2.4$\rightarrow$11.6 (target 2.4/11.6) \\
    IMP / S-AMP, low $\rightarrow$ high glucose (Gooding 2015) & $\times$0.28 / $\times$3.4 (target 0.23/3.4) \\
    Raptor-KO insulin content (Ni 2017) & 50\% of WT (target 50\%) \\
...
\begin{enumerate}\itemsep1pt
  \item Hoxhaj G \textit{et al} (2017) The mTORC1 signaling network senses
  changes in cellular purine nucleotide levels. \textit{Cell Rep} 21:1331.
  \item Oizel K \textit{et al} (2020) Formate induces a metabolic switch in
  nucleotide and energy metabolism. \textit{Cell Death Dis} 11:310.
  \item Rorsman P, Ashcroft FM (2018) Pancreatic $\beta$-cell electrical activity
  and insulin secretion: of mice and men. \textit{Physiol Rev} 98:117--214.
  \item Itoh N, Okamoto H (1980) Translational control of proinsulin synthesis by
  glucose. \textit{Nature} 283:100--102.
  \item Saxton RA, Sabatini DM (2017) mTOR signaling in growth, metabolism, and
  disease. \textit{Cell} 168:960--976.
  \item Kahn SE, Hull RL, Utzschneider KM (2006) Mechanisms linking obesity to
  insulin resistance and type 2 diabetes. \textit{Nature} 444:840--846.
  \item DeFronzo RA (2009) From the triumvirate to the ominous octet: a new
  paradigm for the treatment of type 2 diabetes mellitus. \textit{Diabetes}
  58:773--795.
  \item Roden M, Shulman GI (2019) The integrative biology of type 2 diabetes.
  \textit{Nature} 576:51--60.
  \item Ducker GS, Rabinowitz JD (2017) One-carbon metabolism in health and
  disease. \textit{Cell Metab} 25:27--42.
  \item Brosnan JT \textit{et al} (2018) Lifestyle, metabolite, and genetic
  determinants of formate concentrations in a cross-sectional study in young,
  healthy adults. \textit{Am J Clin Nutr} 107:345--354.
  ...
  \item Pietzke M \textit{et al} (2019) Stratification of cancer and diabetes
  based on circulating levels of formate and glucose. \textit{Cancer Metab} 7:3.
  \item Pietzke M, Meiser J, Vazquez A (2020) Formate metabolism in health and
  disease. \textit{Mol Metab} 33:23--37. (Table~2: formate generation from
  caffeine/aspartame-containing drinks and medications.)
  \item Kawai J \textit{et al} (2019) Discovery of a potent, selective, and orally
  available MTHFD2 inhibitor (DS18561882) with in vivo antitumor activity.
  \textit{J Med Chem} 62:10204.
  \item Yao C \textit{et al} (2025) MTHFD2 drives retinoblastoma progression via
  Notch signaling activation. \textit{Exp Eye Res} 260:110579.
  \item Fraenkel M \textit{et al} (2008) mTOR inhibition by rapamycin prevents
  $\beta$-cell adaptation to hyperglycemia and exacerbates the metabolic state in
  type 2 diabetes. \textit{Diabetes} 57:945.
  \item Carp\'ene C \textit{et al} (2019) Methylamine activates glucose uptake
  in human adipocytes without overpassing action of insulin or stimulating its
  secretion in pancreatic islets. \textit{Medicines} 6:89.
  \item Nunemaker CS \textit{et al} (2006) In vivo and in vitro glucose-induced
  biphasic insulin secretion in the mouse. \textit{Diabetes} 55:441.
  ...
  13:157.
  \item Blandino-Rosano M \textit{et al} (2017) Loss of mTORC1 signalling impairs
  beta-cell maturation and insulin production. \textit{Nat Commun} 8:16014.
  \item Hsu H-C \textit{et al} (2013) Folate deficiency impedes insulin
  biosynthesis in pancreatic beta-cells. \textit{PLoS ONE} 8:e77931.
  \item Pelligra A \textit{et al} (2023) Islet protection at the expense of
  secretion via serine-linked mitochondrial one-carbon metabolism.
  \textit{Cell Rep} 42:112615.
  \item Takase M \textit{et al} (2025) Association of circulating metabolites and
  polygenic risk score with incident type 2 diabetes: a prospective
  community-based cohort study. \textit{Cardiovasc Diabetol} 24:335.
  ...
  \item Tramonti A \textit{et al} (2021) Metformin is a pyridoxal-5$'$-phosphate
  (PLP)-competitive inhibitor of SHMT2. \textit{Cancers} 13:4009.
  \item Etchegaray JI \textit{et al} (2023) Phagocytosis in the retina promotes
  local insulin production in the eye. \textit{Nat Metab} 5:207.
  \item Ruberte J \textit{et al} (2004) Increased ocular levels of IGF-1 in
  transgenic mice lead to diabetes-like eye disease. \textit{J Clin Invest}
  113:1149.
  \item Villacampa P \textit{et al} (2013) Insulin-like growth factor I-induced
  chronic gliosis and retinal stress lead to neurodegeneration.
  \textit{J Biol Chem} 288:17631.
  \item Ding M \textit{et al} (2014) Caffeinated and decaffeinated coffee
  consumption and risk of type 2 diabetes: a dose-response meta-analysis.
  \textit{Diabetes Care} 37:569.
  \item Imamura F \textit{et al} (2015) Consumption of sugar sweetened beverages,
  artificially sweetened beverages, and fruit juice and incidence of type 2
  diabetes. \textit{BMJ} 351:h3576.
  \item Li C \textit{et al} (2024) Meat consumption and incident type 2 diabetes:
  a federated meta-analysis of 1.97 million adults. \textit{Lancet Diabetes
  Endocrinol} 12:619.
  \item Delpino FM, Figueiredo LM (2022) Does creatine supplementation improve
  glycemic control and insulin resistance? A meta-analysis. \textit{Clin Nutr
  ESPEN} 47:128.
  \item Ben-Sahra I \textit{et al} (2016) mTORC1 induces purine synthesis through
  control of the mitochondrial tetrahydrofolate cycle. \textit{Science} 351:728.
  \item Leontieva OV, Demidenko ZN, Blagosklonny MV (2014) Rapamycin reverses
  insulin resistance (IRS-1 serine phosphorylation) in retinal pigment epithelial
  cells. \textit{Cell Death Dis} 5:e1214.
  \item Altaweel MM \textit{et al} (2009) Ocular and systemic safety evaluation of
  calcium formate as a dietary supplement. \textit{J Ocul Pharmacol Ther} 25:223.
  \item Vazquez A (2020) \textit{The Spice of Life}.
  \item Vazquez A, An allosteric metabolite glue couples AICAR inversely to
  one-carbon availability (submitted) --- Formate-NUDT5.
\end{enumerate}
